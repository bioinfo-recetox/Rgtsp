...
Graph &operator+=(Graph &A, const Graph &B
{
    typename Graph::node_iterator p;
    for_iterator(p,B)
    { 
        const Graph::vertex &v = Graph::node(p);
        Graph::edge_set &S = v.out_neighbors();
...
Graph &operator+=(Graph &A, const Graph &B
{
    typename Graph::IteratorType p;
    // run over vertciesn of B
    for_each(p,B)
    { 
        const typename Graph::VertexType &v = Graph::Vertex(p);
        typename Graph::EdgeSetType &S = Graph::OutEdges(p);
        Graph::EdgeSetType::const_iterator e;
        for_iterator(e, S)
        {
            A.InsertEdge(v, *e);
        }
